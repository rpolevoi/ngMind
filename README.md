...
The answer is to replace the native or hybrid mobile app as we know it with what Google calls the __"progressive web app__." This model for development, which works right now in primitive examples, involves matching the user experience of native mobile apps in a __web app living in the browser__. Instant loading of at least the initial interface from browser storage. Offline capabilities, especially during wireless service interruptions. The appearance of a dedicated app, with browser features hidden unless sought. And, most importantly, the killer feature current mobile apps cannot pretend to offer: __linking of urls assigned to application state views__. A user can copy an url, even from deep within the app, and send it in an email as a link, where it can move to social media or web sites. And Google can find such links and make them searchable. All this because everything now runs in the browser, with acesss to local storage.

Should you doubt if this is really the most salient motive for Google with Angular 2 -- as the __ideal platform for displacing native mobile apps with searchable web apps that run everywhere__ -- check out the primary position given to progressive web apps on the Angular website Features page. In fact, Google goes much further, stressing that Angular 2 is the ideal vehicle for unifying even current web and native app development, so that we don't have to write three different apps for the same client -- browser, iOS and Android -- or at least don't have to write them all from scratch.
...
My purpose here has been to provide the __graduated, yet holistic picture__ that I wish I had when jumping in. Our goal is to sense how Angular 2 hangs together and provides something far more powerful and interesting than the sum of its manifold parts.
...
Angular 2 was designed to be written in TypeScript. Does this really mean having to learn a new language or dialect?
